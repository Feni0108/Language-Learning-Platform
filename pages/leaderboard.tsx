...
export default function Leaderboard({leaderBoard}: LeaderBoardType, {userSettings}: SettingsProps) {
  ...
  return (
      ...
      <div>
        {!isLoading && session && (
            <>{t('Signed_in_as')} {session.user?.email ? session.user.email : session.user!.username}
              <br/>
              <div>
                <button
                    onClick={isPoint ? undefined : () => {
                      setIsPoint(true), setIsLoading(true)
                    }}
                >Points
                </button>
                <button
                    onClick={!isPoint ? undefined : () => {
                      setIsPoint(false), setIsLoading(true)
                    }}
                >Strike
                </button>
